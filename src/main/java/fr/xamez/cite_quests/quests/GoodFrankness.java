...
public class GoodFrankness {

    private final Quest quest;
    public final static String ID = "A la bonne franquette";

    public GoodFrankness(CiteQuestCore citeQuestCore){
        ...
        switch (step){
            ...
            case 1:
                if (!Manager.playerDialogues.contains(p.getUniqueId())) {
                    ...
                    if (p.getInventory().contains(wheat)) {
                        if (!Manager.playerDialogues.contains(p.getUniqueId())) {
                            Manager.playerDialogues.add(p.getUniqueId());
